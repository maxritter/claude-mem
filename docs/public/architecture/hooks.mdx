...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const response = await fetch(
    `http://localhost:37777/api/context/inject?project=${project}`
  );
...
<Note>
  As of Claude Code 2.1.0 (ultrathink update), SessionStart hooks no longer
  display user-visible messages. Context is silently injected via
...
<Note>
  The same `session_id` flows through ALL hooks in a conversation. The
  `createSDKSession` call is idempotent - it returns the existing session for
...
<Warning>
  Never generate your own session IDs. Always use the `session_id` provided by
  the IDE - this is the source of truth for linking all data together.
</Warning>
