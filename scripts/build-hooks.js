...
async function buildHooks() {
  ...
  try {
    ...
    // External packages with native bindings need to be installed at runtime
    ...
    // Build React viewer with Vite
    console.log('\n📋 Building React viewer with Vite...');
    const { spawn } = await import('child_process');
    const viewerBuild = spawn('npm', ['run', 'build:viewer'], { stdio: 'inherit' });
    ...
    // Build worker wrapper (Windows-compatible process manager)
    ...
    console.log(`   - Worker Wrapper: worker-wrapper.cjs`);
