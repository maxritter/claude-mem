...
`+JSON.stringify(o,null,2):l=" "+this.formatData(o));let _="";if(r){let{sessionId:E,memorySessionId:h,correlationId:f,...m}=r;Object.keys(m).length>0&&(_=` {${Object.entries(m).map(([T,C])=>`${T}=${C}`).join(", ")}}`)}let g=`[${i}] [${d}] [${a}] ${p}${s}${_}${l}`;if(this.logFilePath)try{(0,N.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),l=p?`${p[1].split("/").pop()}:${p[2]}`:"unknown",_={...s,location:l};return this.warn(e,`[HAPPY-PATH] ${t}`,_,r),o}},u=new Y;var Rt={};function ht(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,pe.fileURLToPath)(Rt.url))}var Ot=ht(),M=I.get("CLAUDE_MEM_DATA_DIR"),D=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,ce.homedir)(),".claude"),Wt=(0,b.join)(M,"archives"),Gt=(0,b.join)(M,"logs"),Yt=(0,b.join)(M,"trash"),Vt=(0,b.join)(M,"backups"),Kt=(0,b.join)(M,"modes"),qt=(0,b.join)(M,"settings.json"),ue=(0,b.join)(M,"claude-mem.db"),Jt=(0,b.join)(M,"vector-db"),zt=(0,b.join)(D,"settings.json"),Qt=(0,b.join)(D,"commands"),Zt=(0,b.join)(D,"CLAUDE.md"),es=(0,b.join)(D,".credentials.json"),Ct=(0,b.join)(D,"plugins"),ts=(0,b.join)(Ct,"marketplaces","customable");function le(n){(0,w.mkdirSync)(n,{recursive:!0})}function me(){return(0,b.join)(Ot,"..")}var F=class{db;constructor(e=ue){e!==":memory:"&&le(M),this.db=new _e.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn()}initializeSchema(){this.db.run(`
    ...
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),r||null,o,a,d);return{id:Number(l.lastInsertRowid),createdAtEpoch:d}}storeSummary(e,t,s,r,o=0,i){let d=i??Date.now(),a=new Date(d).toISOString(),l=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,r||null,o,a,d);return{id:Number(l.lastInsertRowid),createdAtEpoch:d}}storeObservations(e,t,s,r,o,i=0,d,a){let p=d??Date.now(),l=new Date(p).toISOString();return this.db.transaction(()=>{let g=[],E=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch, git_branch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let f of s){let m=E.run(e,t,f.type,f.title,f.subtitle,JSON.stringify(f.facts),f.narrative,JSON.stringify(f.concepts),JSON.stringify(f.files_read),JSON.stringify(f.files_modified),o||null,i,l,p,a||null);g.push(Number(m.lastInsertRowid))}let h=null;if(r){let m=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,r.request,r.investigated,r.learned,r.completed,r.next_steps,r.notes,o||null,i,l,p);h=Number(m.lastInsertRowid)}return{observationIds:g,summaryId:h,createdAtEpoch:p}})()}storeObservationsAndMarkComplete(e,t,s,r,o,i,d,a=0,p){let l=p??Date.now(),_=new Date(l).toISOString();return this.db.transaction(()=>{let E=[],h=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let S of s){let T=h.run(e,t,S.type,S.title,S.subtitle,JSON.stringify(S.facts),S.narrative,JSON.stringify(S.concepts),JSON.stringify(S.files_read),JSON.stringify(S.files_modified),d||null,a,_,l);E.push(Number(T.lastInsertRowid))}let f;if(r){let T=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,r.request,r.investigated,r.learned,r.completed,r.next_steps,r.notes,d||null,a,_,l);f=Number(T.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(l,o),{observationIds:E,summaryId:f,createdAtEpoch:l}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:r,project:o}=t,i=s==="date_asc"?"ASC":"DESC",d=r?`LIMIT ${r}`:"",a=e.map(()=>"?").join(","),p=[...e],l=o?`WHERE id IN (${a}) AND project = ?`:`WHERE id IN (${a})`;return o&&p.push(o),this.db.prepare(`
    ...
    `).all(...p)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:r,project:o}=t,i=s==="date_asc"?"ASC":"DESC",d=r?`LIMIT ${r}`:"",a=e.map(()=>"?").join(","),p=[...e],l=o?"AND s.project = ?":"";return o&&p.push(o),this.db.prepare(`
      ...
      ${d}
    `).all(...p)}getTimelineAroundTimestamp(e,t=10,s=10,r){return this.getTimelineAroundObservation(null,e,t,s,r)}getTimelineAroundObservation(e,t,s=10,r=10,o){let i=o?"AND project = ?":"",d=o?[o]:[],a,p;if(e!==null){let m=`
...
`}function et(n,e,t,s){let r=[];return s?r.push(...Xe(n)):r.push(...Ie(n)),s?r.push(...Be()):r.push(...Ae()),s?r.push(...He()):r.push(...ye()),s?r.push(...We()):r.push(...Le()),j(t)&&(s?r.push(...Ge(e,t)):r.push(...De(e,t))),r}var te=y(require("path"),1);function W(n){if(!n)return[];try{let e=JSON.parse(n);return Array.isArray(e)?e:[]}catch(e){return u.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:n?.substring(0,50)},e),[]}}function st(n){return new Date(n).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function rt(n){return new Date(n).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function nt(n){return new Date(n).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function tt(n,e){return te.default.isAbsolute(n)?te.default.relative(e,n):n}function ot(n,e,t){let s=W(n);if(s.length>0)return tt(s[0],e);if(t){let r=W(t);if(r.length>0)return tt(r[0],e)}return"General"}function At(n){let e=new Map;for(let s of n){let r=s.type==="observation"?s.data.created_at:s.data.displayTime,o=nt(r);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,r)=>{let o=new Date(s[0]).getTime(),i=new Date(r[0]).getTime();return o-i});return new Map(t)}function yt(n,e){return e.fullObservationField==="narrative"?n.narrative:n.facts?W(n.facts).join(`
`):null}function Lt(n,e,t,s,r,o){let i=[];o?i.push(...Ye(n)):i.push(...ve(n));let d=null,a="",p=!1;for(let l of e)if(l.type==="summary"){p&&(i.push(""),p=!1,d=null,a="");let _=l.data,g=st(_.displayTime);o?i.push(...Je(_,g)):i.push(...$e(_,g))}else{let _=l.data,g=ot(_.files_modified,r,_.files_read),E=rt(_.created_at),h=E!==a,f=h?E:"";a=E;let m=t.has(_.id);if(g!==d&&(p&&i.push(""),o?i.push(...Ve(g)):i.push(...xe(g)),d=g,p=!0),m){let S=yt(_,s);o?i.push(...qe(_,E,h,S,s)):(p&&!o&&(i.push(""),p=!1),i.push(...ke(_,f,S,s)),d=null)}else o?i.push(Ke(_,E,h,s)):i.push(Ue(_,f,s))}return p&&i.push(""),i}function it(n,e,t,s,r){let o=[],i=At(n);for(let[d,a]of i)o.push(...Lt(d,a,e,t,s,r));return o}function at(n,e,t){return!(!n.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function dt(n,e){let t=[];return e?(t.push(...U("Investigated",n.investigated,c.blue)),t.push(...U("Learned",n.learned,c.yellow)),t.push(...U("Completed",n.completed,c.green)),t.push(...U("Next Steps",n.next_steps,c.magenta))):(t.push(...x("Investigated",n.investigated)),t.push(...x("Learned",n.learned)),t.push(...x("Completed",n.completed)),t.push(...x("Next Steps",n.next_steps))),t}function ct(n,e){return e?ze(n):we(n)}function pt(n,e,t){return!j(e)||n.totalDiscoveryTokens<=0||n.savings<=0?[]:t?Qe(n.totalDiscoveryTokens,n.totalReadTokens):Fe(n.totalDiscoveryTokens,n.totalReadTokens)}var Dt=ut.default.join((0,lt.homedir)(),".claude","plugins","marketplaces","customable","plugin",".install-version");function vt(){try{return new F}catch(n){if(n.code==="ERR_DLOPEN_FAILED"){try{(0,mt.unlinkSync)(Dt)}catch(e){u.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return u.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw n}}function xt(n,e){return e?Ze(n):Pe(n)}function Ut(n,e,t,s,r,o,i){let d=[],a=J(e);d.push(...et(n,a,s,i));let p=t.slice(0,s.sessionCount),l=Re(p,t),_=ee(e,l),g=Me(e,s.fullObservationCount);d.push(...it(_,g,s,r,i));let E=t[0],h=e[0];at(s,E,h)&&d.push(...dt(E,i));let f=Z(e,s,o,r);return d.push(...ct(f,i)),d.push(...pt(a,s,i)),d.join(`
`).trimEnd()}async function se(n,e=!1){let t=V(),s=n?.cwd??process.cwd(),r=ge(s),o=n?.projects||[r],i=vt();if(!i)return"";try{let d=o.length>1?Oe(i,o,t):z(i,r,t),a=o.length>1?Ce(i,o,t):Q(i,r,t);return d.length===0&&a.length===0?xt(r,e):Ut(r,d,a,t,s,n?.session_id,e)}finally{i.close()}}0&&(module.exports={generateContext});
