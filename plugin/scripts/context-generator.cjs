...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let _="";if(r){let{sessionId:E,memorySessionId:h,correlationId:f,...l}=r;Object.keys(l).length>0&&(_=` {${Object.entries(l).map(([T,C])=>`${T}=${C}`).join(", ")}}`)}let g=`[${i}] [${d}] [${a}] ${p}${s}${_}${m}`;if(this.logFilePath)try{(0,I.appendFileSync)(this.logFilePath,g+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=p?`${p[1].split("/").pop()}:${p[2]}`:"unknown",_={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,_,r),o}},u=new Y;var Rt={};function ht(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,pe.fileURLToPath)(Rt.url))}var Ot=ht(),N=y.get("CLAUDE_MEM_DATA_DIR"),v=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,ce.homedir)(),".claude"),Wt=(0,b.join)(N,"archives"),Gt=(0,b.join)(N,"logs"),Yt=(0,b.join)(N,"trash"),Vt=(0,b.join)(N,"backups"),Kt=(0,b.join)(N,"modes"),qt=(0,b.join)(N,"settings.json"),ue=(0,b.join)(N,"claude-mem.db"),Jt=(0,b.join)(N,"vector-db"),zt=(0,b.join)(v,"settings.json"),Qt=(0,b.join)(v,"commands"),Zt=(0,b.join)(v,"CLAUDE.md"),es=(0,b.join)(v,".credentials.json"),Ct=(0,b.join)(v,"plugins"),ts=(0,b.join)(Ct,"marketplaces","customable");function me(n){(0,w.mkdirSync)(n,{recursive:!0})}function le(){return(0,b.join)(Ot,"..")}var F=class{db;constructor(e=ue){e!==":memory:"&&me(N),this.db=new _e.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn()}initializeSchema(){this.db.run(`
    ...
    `).all(e)}getObservationById(e){return this.db.prepare(`
      ...
      FROM observations
      WHERE id = ?
    `).get(e)||null}getObservationsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:r,project:o,type:i,concepts:d,files:a}=t,p=s==="date_asc"?"ASC":"DESC",m=r?`LIMIT ${r}`:"",_=e.map(()=>"?").join(","),g=[...e],E=[];if(o&&(E.push("project = ?"),g.push(o)),i)if(Array.isArray(i)){let l=i.map(()=>"?").join(",");E.push(`type IN (${l})`),g.push(...i)}else E.push("type = ?"),g.push(i);if(d){let l=Array.isArray(d)?d:[d],S=l.map(()=>"EXISTS (SELECT 1 FROM json_each(concepts) WHERE value = ?)");g.push(...l),E.push(`(${S.join(" OR ")})`)}if(a){let l=Array.isArray(a)?a:[a],S=l.map(()=>"(EXISTS (SELECT 1 FROM json_each(files_read) WHERE value LIKE ?) OR EXISTS (SELECT 1 FROM json_each(files_modified) WHERE value LIKE ?))");l.forEach(T=>{g.push(`%${T}%`,`%${T}%`)}),E.push(`(${S.join(" OR ")})`)}let h=E.length>0?`WHERE id IN (${_}) AND ${E.join(" AND ")}`:`WHERE id IN (${_})`;return this.db.prepare(`
      SELECT *
      FROM observations
      ${h}
    ...
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),r||null,o,a,d);return{id:Number(m.lastInsertRowid),createdAtEpoch:d}}storeSummary(e,t,s,r,o=0,i){let d=i??Date.now(),a=new Date(d).toISOString(),m=this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,r||null,o,a,d);return{id:Number(m.lastInsertRowid),createdAtEpoch:d}}storeObservations(e,t,s,r,o,i=0,d,a){let p=d??Date.now(),m=new Date(p).toISOString();return this.db.transaction(()=>{let g=[],E=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch, git_branch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let f of s){let l=E.run(e,t,f.type,f.title,f.subtitle,JSON.stringify(f.facts),f.narrative,JSON.stringify(f.concepts),JSON.stringify(f.files_read),JSON.stringify(f.files_modified),o||null,i,m,p,a||null);g.push(Number(l.lastInsertRowid))}let h=null;if(r){let l=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,r.request,r.investigated,r.learned,r.completed,r.next_steps,r.notes,o||null,i,m,p);h=Number(l.lastInsertRowid)}return{observationIds:g,summaryId:h,createdAtEpoch:p}})()}storeObservationsAndMarkComplete(e,t,s,r,o,i,d,a=0,p){let m=p??Date.now(),_=new Date(m).toISOString();return this.db.transaction(()=>{let E=[],h=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let S of s){let T=h.run(e,t,S.type,S.title,S.subtitle,JSON.stringify(S.facts),S.narrative,JSON.stringify(S.concepts),JSON.stringify(S.files_read),JSON.stringify(S.files_modified),d||null,a,_,m);E.push(Number(T.lastInsertRowid))}let f;if(r){let T=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(e,t,r.request,r.investigated,r.learned,r.completed,r.next_steps,r.notes,d||null,a,_,m);f=Number(T.lastInsertRowid)}return this.db.prepare(`
      ...
      `).run(m,o),{observationIds:E,summaryId:f,createdAtEpoch:m}})()}getSessionSummariesByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:r,project:o}=t,i=s==="date_asc"?"ASC":"DESC",d=r?`LIMIT ${r}`:"",a=e.map(()=>"?").join(","),p=[...e],m=o?`WHERE id IN (${a}) AND project = ?`:`WHERE id IN (${a})`;return o&&p.push(o),this.db.prepare(`
    ...
    `).all(...p)}getUserPromptsByIds(e,t={}){if(e.length===0)return[];let{orderBy:s="date_desc",limit:r,project:o}=t,i=s==="date_asc"?"ASC":"DESC",d=r?`LIMIT ${r}`:"",a=e.map(()=>"?").join(","),p=[...e],m=o?"AND s.project = ?":"";return o&&p.push(o),this.db.prepare(`
      ...
      ${d}
    `).all(...p)}getTimelineAroundTimestamp(e,t=10,s=10,r){return this.getTimelineAroundObservation(null,e,t,s,r)}getTimelineAroundObservation(e,t,s=10,r=10,o){let i=o?"AND project = ?":"",d=o?[o]:[],a,p;if(e!==null){let l=`
      ...
      `,S=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let T=this.db.prepare(l).all(e,...d,s+1),C=this.db.prepare(S).all(e,...d,r+1);if(T.length===0&&C.length===0)return{observations:[],sessions:[],prompts:[]};a=T.length>0?T[T.length-1].created_at_epoch:t,p=C.length>0?C[C.length-1].created_at_epoch:t}catch(T){return u.error("DB","Error getting boundary observations",void 0,{error:T,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let l=`
      ...
      `,S=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let T=this.db.prepare(l).all(t,...d,s),C=this.db.prepare(S).all(t,...d,r+1);if(T.length===0&&C.length===0)return{observations:[],sessions:[],prompts:[]};a=T.length>0?T[T.length-1].created_at_epoch:t,p=C.length>0?C[C.length-1].created_at_epoch:t}catch(T){return u.error("DB","Error getting boundary timestamps",void 0,{error:T,project:o}),{observations:[],sessions:[],prompts:[]}}}let m=`
      SELECT *
      FROM observations
...
`}function et(n,e,t,s){let r=[];return s?r.push(...Xe(n)):r.push(...ye(n)),s?r.push(...Be()):r.push(...Ae()),s?r.push(...He()):r.push(...Me()),s?r.push(...We()):r.push(...Le()),j(t)&&(s?r.push(...Ge(e,t)):r.push(...ve(e,t))),r}var te=M(require("path"),1);function W(n){if(!n)return[];try{let e=JSON.parse(n);return Array.isArray(e)?e:[]}catch(e){return u.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:n?.substring(0,50)},e),[]}}function st(n){return new Date(n).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function rt(n){return new Date(n).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function nt(n){return new Date(n).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function tt(n,e){return te.default.isAbsolute(n)?te.default.relative(e,n):n}function ot(n,e,t){let s=W(n);if(s.length>0)return tt(s[0],e);if(t){let r=W(t);if(r.length>0)return tt(r[0],e)}return"General"}function At(n){let e=new Map;for(let s of n){let r=s.type==="observation"?s.data.created_at:s.data.displayTime,o=nt(r);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,r)=>{let o=new Date(s[0]).getTime(),i=new Date(r[0]).getTime();return o-i});return new Map(t)}function Mt(n,e){return e.fullObservationField==="narrative"?n.narrative:n.facts?W(n.facts).join(`
`):null}function Lt(n,e,t,s,r,o){let i=[];o?i.push(...Ye(n)):i.push(...De(n));let d=null,a="",p=!1;for(let m of e)if(m.type==="summary"){p&&(i.push(""),p=!1,d=null,a="");let _=m.data,g=st(_.displayTime);o?i.push(...Je(_,g)):i.push(...$e(_,g))}else{let _=m.data,g=ot(_.files_modified,r,_.files_read),E=rt(_.created_at),h=E!==a,f=h?E:"";a=E;let l=t.has(_.id);if(g!==d&&(p&&i.push(""),o?i.push(...Ve(g)):i.push(...xe(g)),d=g,p=!0),l){let S=Mt(_,s);o?i.push(...qe(_,E,h,S,s)):(p&&!o&&(i.push(""),p=!1),i.push(...Ue(_,f,S,s)),d=null)}else o?i.push(Ke(_,E,h,s)):i.push(ke(_,f,s))}return p&&i.push(""),i}function it(n,e,t,s,r){let o=[],i=At(n);for(let[d,a]of i)o.push(...Lt(d,a,e,t,s,r));return o}function at(n,e,t){return!(!n.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function dt(n,e){let t=[];return e?(t.push(...k("Investigated",n.investigated,c.blue)),t.push(...k("Learned",n.learned,c.yellow)),t.push(...k("Completed",n.completed,c.green)),t.push(...k("Next Steps",n.next_steps,c.magenta))):(t.push(...x("Investigated",n.investigated)),t.push(...x("Learned",n.learned)),t.push(...x("Completed",n.completed)),t.push(...x("Next Steps",n.next_steps))),t}function ct(n,e){return e?ze(n):we(n)}function pt(n,e,t){return!j(e)||n.totalDiscoveryTokens<=0||n.savings<=0?[]:t?Qe(n.totalDiscoveryTokens,n.totalReadTokens):Fe(n.totalDiscoveryTokens,n.totalReadTokens)}var vt=ut.default.join((0,mt.homedir)(),".claude","plugins","marketplaces","customable","plugin",".install-version");function Dt(){try{return new F}catch(n){if(n.code==="ERR_DLOPEN_FAILED"){try{(0,lt.unlinkSync)(vt)}catch(e){u.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return u.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw n}}function xt(n,e){return e?Ze(n):Pe(n)}function kt(n,e,t,s,r,o,i){let d=[],a=J(e);d.push(...et(n,a,s,i));let p=t.slice(0,s.sessionCount),m=Re(p,t),_=ee(e,m),g=Ne(e,s.fullObservationCount);d.push(...it(_,g,s,r,i));let E=t[0],h=e[0];at(s,E,h)&&d.push(...dt(E,i));let f=Z(e,s,o,r);return d.push(...ct(f,i)),d.push(...pt(a,s,i)),d.join(`
`).trimEnd()}async function se(n,e=!1){let t=V(),s=n?.cwd??process.cwd(),r=ge(s),o=n?.projects||[r],i=Dt();if(!i)return"";try{let d=o.length>1?Oe(i,o,t):z(i,r,t),a=o.length>1?Ce(i,o,t):Q(i,r,t);return d.length===0&&a.length===0?xt(r,e):kt(r,d,a,t,s,n?.session_id,e)}finally{i.close()}}0&&(module.exports={generateContext});
